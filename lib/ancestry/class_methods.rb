module Ancestry
  module ClassMethods
    ...
    def build_ancestry_from_parent_ids! client_id = nil, column=:parent_id, parent_id = nil, ancestor_ids = []
      unscoped_where do |scope|
        scope.where(:client_id => client_id, column => parent_id).find_each do |node|
          node.without_ancestry_callbacks do
            node.update_attribute :ancestor_ids, ancestor_ids
          end
          build_ancestry_from_parent_ids! client_id, column, node.id, ancestor_ids + [node.id]
        end
      end
    end
