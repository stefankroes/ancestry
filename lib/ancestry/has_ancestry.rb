class << ActiveRecord::Base
  def has_ancestry options = {}
    ...
    scope :ordered_by_ancestry, reorder("(CASE WHEN #{quoted_table_name}.#{connection.quote_column_name(ancestry_column)} IS NULL THEN 0 ELSE 1 END), #{quoted_table_name}.#{connection.quote_column_name(ancestry_column)}")
    scope :ordered_by_ancestry_and, lambda { |order| reorder("(CASE WHEN #{quoted_table_name}.#{connection.quote_column_name(ancestry_column)} IS NULL THEN 0 ELSE 1 END), #{quoted_table_name}.#{connection.quote_column_name(ancestry_column)}, #{order}") }
