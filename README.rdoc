...
To get the arranged nodes as a serializable hash instead of a nested hash:

  TreeNode.arrange_to_hash
...
And to output as json, just add to_json afterwards:

  TreeNode.arrange_to_hash.to_json
...
You can pass an environment variable for the database to test against (e.g. db=mysql). By default, the test suite runs against the latest Active Record version. You can run against Active Record 3-0 or 3-1 as follows:
- run <tt>bundle --gemfile Gemfile.rails-<version></tt>
- run <tt>rake [test] BUNDLE_GEMFILE=Gemfile.rails-<version></tt>

To run the test suite multiple times against several databases and all supported Active Record versions, run <tt>rake test_all</tt>
